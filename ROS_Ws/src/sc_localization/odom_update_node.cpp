...
bool init = false;
ros::Publisher pub_;
float max_update_dist_update, tag_void_frames;
...
void aprilCallback(const apriltag_ros::AprilTagDetectionArray::ConstPtr& msg)
{
    ...
    double odom_cam_x, odom_cam_y, odom_cam_z;
    double tag_cam_x =0, tag_cam_y =0, tag_cam_z =0;
    double tag_x =0, tag_y =0;
    tf::Quaternion odom_cam_q, tag_cam_q, tag_q;
    ...
    if (!msg->detections.empty()){
        ...
        geometry_msgs::PoseWithCovarianceStamped tag_pose = msg->detections[nearest_tag_id].pose;
        tag_pose.header.frame_id = "tag_42";
        pub_.publish(tag_pose);
...
//            ros::Duration(1.0).sleep();
        ...
        odom_cam_z = odom_cam_transform.getOrigin().z();
        odom_cam_q = odom_cam_transform.getRotation();

        /// Obtain information from map to tag
        std::stringstream ss;
        ss << msg->detections[nearest_tag_id].id[0];
...
//            ros::Duration(1.0).sleep();
        ...
        tag_q = map_tag_transform.getRotation();
        ...
        tf::Matrix3x3 tag_r(tag_q);
        tf::Matrix3x3 odom_r = tag_r*tag_cam_r*(odom_cam_r.transpose());

        tf::Vector3 tag_cam_v(tag_cam_x,tag_cam_y,tag_cam_z);
        double cam_x = (tag_r*tag_cam_v)[0] + tag_x;
        double cam_y = (tag_r*tag_cam_v)[1] + tag_y;

        tf::Vector3 cam_v(cam_x, cam_y, odom_cam_z);
        tf::Vector3 odom_cam_v(odom_cam_x,odom_cam_y,odom_cam_z);
        ...
        if (init && fabs((odom_v_old[0]*odom_v_old[0] +odom_v_old[1]*odom_v_old[1])-(odom_v[0]*odom_v[0] +odom_v[1]*odom_v[1]))> max_update_dist_update && not_seeing_tag_counter < tag_void_frames){
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle nh;
  nh.param<float>("max_square_dist_update", max_update_dist_update, 0.4);
  nh.param<float>("tag_void_frames", tag_void_frames, 100);
  listener = new tf::TransformListener(ros::Duration(1));
  pub_ = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("tag",10);
