...
class LocalPlanner{
 public:
  LocalPlanner(ros::NodeHandle& given_nh):nh(given_nh),plan_(false), prune_index_(0){
    ...
    nh.param<double>("p_coeff", p_coeff_, 1.0);
    ...
    nh.param<double>("prune_ahead_distance_local", prune_ahead_dist_local_, 0.1);
    ...
    targetPoint_pub_ = nh.advertise<geometry_msgs::PoseStamped>("/target_point", 1);
 ...
 private:
  ...
  void FollowTraj(const geometry_msgs::PoseStamped& robot_pose,
    ...
    cmd_vel.linear.y = max_speed_*(1.0-std::abs(diff_yaw)/(max_angle_diff_));
    cmd_vel.linear.x = 0;
    ...
    GetGlobalRobotPose(tf_listener_, global_path_.header.frame_id, global_robot_pose);
    FindPoseLookahead(robot_pose, traj, prune_local_index_, prune_ahead_dist_local_);

    geometry_msgs::PoseStamped targetPose;
    targetPose.pose = traj.poses[prune_local_index_].pose;
    targetPose.header.stamp=ros::Time::now();
    targetPose.header.frame_id="map";
    targetPoint_pub_.publish(targetPose);
    //tmp_pose.header.frame_id = global_frame_;

    //TransformPose(global2path_transform_, robot_pose, tmp_pose);

    double diffX = robot_pose.pose.position.x - traj.poses[prune_local_index_].pose.position.x;
    double diffY = robot_pose.pose.position.y - traj.poses[prune_local_index_].pose.position.y;

    double distance = std::hypot(diffX, diffY);
    double theta = std::atan2(diffY, diffX);

    double robotYaw = GetYawFromOrientation(robot_pose.pose.orientation);

    double yawSin = std::sin(robotYaw);
    double yawCos = std::cos(robotYaw);

    double xMag = distance * std::cos(theta);
    double yMag = distance * std::sin(theta);

    xMag = (yawCos * xMag - yawSin * yMag);
    yMag = (yawSin * xMag - yawCos * yMag);

    ROS_INFO("poseX %f poseY %f rPoseX %f rPoseY %f diffX: %f, diffY: %f distance %f theta %f yaw %f %f,%f", traj.poses[prune_local_index_].pose.position.x, traj.poses[prune_local_index_].pose.position.y, robot_pose.pose.position.x, robot_pose.pose.position.y, diffX, diffY, distance, theta, robotYaw, xMag, yMag);
 ...
 private:
  ...
  void FindNearstPose(const geometry_msgs::PoseStamped& robot_pose,const nav_msgs::Path& path, int& prune_index, double prune_ahead_dist = 0.3){
  ...
  void FindPoseLookahead(const geometry_msgs::PoseStamped& robot_pose,const nav_msgs::Path& path, int& prune_index, double prune_ahead_dist = 0.3){
    double distance = 0;
    int temp_index = 0;

    ROS_INFO("%i %i", prune_index, (int) path.poses.size());
    
    while(temp_index < (int) path.poses.size()){
        double newDistance = GetEuclideanDistance(robot_pose, path.poses[temp_index]);
        ROS_INFO("%i with %f", temp_index, distance);

        if(newDistance > distance){
            ROS_INFO("DSHADSAD");
            prune_index = temp_index;
            distance = newDistance;
        }

        if(distance > prune_ahead_dist){
          ROS_INFO("dasdadsa");
          prune_index = temp_index;
          prune_index = std::min(prune_index, (int)(path.poses.size()-1));
          return;
        ...
        //std::cout << std::to_string(temp_index) << std::to_string(distance) << std::endl;
  ...
  ros::Publisher targetPoint_pub_;

  bool plan_;
  int prune_index_;
  int prune_local_index_;
  ...
  double prune_ahead_dist_local_;
  int plan_freq_;
