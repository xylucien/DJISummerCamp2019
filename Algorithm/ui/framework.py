...
while True:
	if TEST:
				...
				if self.state!=CaptureState["free"]:
					...
					for castle in self.castle_connected[self.state]:
						if castle.state==self.state:
							enbuf+=1
					self.energy=enbuf
					self.strong = True if self.energy >= 3 else False
		...
		class Mp(object):
			...
			def calculate_pts(self):
				...
				for i in self.blocks:
					if i.no in [2,6,27,31,35,56,60]:
						continue
					if i.state == CaptureState["blue"]:
						b_pts += 20 if i.strong else 15
					if i.state == CaptureState["red"]:
						r_pts += 20 if i.strong else 15
				for j in self.castles:
					if j.state == CaptureState["blue"]:
						b_pts+=50
					if j.state == CaptureState["red"]:
						r_pts+=50


			def BCU(self,blk,num,blst):
				if 0<=num<=62 and blk.state!=CaptureState["free"] and blst[num].state==blk.state:
		...
		def move(direction,lth,team=CaptureState["blue"]):
			global r_pts_deducted, b_pts_deducted
			time.sleep(MOVE_TIME)
			robots[team.value-1].movpoz(direction,lth,detect=True,opppoz=robots[{CaptureState["blue"]:1,CaptureState["red"]:0}[team]].poz)
			if mp.blocks[robots[team.value-1].poz[0] + 9*robots[team.value-1].poz[1]].state != CaptureState["free"] and team.value != mp.blocks[robots[team.value-1].poz[0] + 9*robots[team.value-1].poz[1]].state.value:
				if team.value == 1:
					b_pts_deducted+=5
				else:
					r_pts_deducted+=5
		...
		print("")
		print("--------------------OPEN SOURCE--------------------")
		print("RoboMaster 2019 Summer Camp Game Simulator   By:AAA_shuibiao   QQ:1308499101  Calibrating: Lucien")
		print("")
			...
			for event in pygame.event.get():
				...
				if event.type==KEYDOWN:
					if event.key==K_ESCAPE:
						pygame.quit()
						exit()
					#TODO:adding time module and step module here
					if KEYBOARD_DRIVE_ROBOT:
						...
						mp.calculate_pts()
						print(b_pts_deducted, r_pts_deducted)
						print(b_pts, r_pts)
