...
void DetectTask(void const *pvParameters) {
  ...
  while (1) {
    ...
    for (int i = 0; i < errorListLength; i++) {
      ...
      if (systemTime - errorList[i].newTime > errorList[i].setOfflineTime) {
      ...
      } else if (systemTime - errorList[i].worktime <
                 errorList[i].setOnlineTime) {
        ...
        if (errorList[i].newTime > errorList[i].lastTime) {
          errorList[i].frequency =
              configTICK_RATE_HZ /
              (fp32)(errorList[i].newTime - errorList[i].lastTime);
        }
      }
    }
...
//返回对应的设备是否存在错误
bool_t toe_is_error(uint8_t err) { return (errorList[err].errorExist == 1); }
...
const error_t *getErrorListPoint(void) { return errorList; }
static void DetectDisplay(uint8_t num) {
