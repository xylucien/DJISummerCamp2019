...
extern void chassis_normal_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set,
                                   chassis_move_t *chassis_move_rc_to_vector);
extern void chassis_auto_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set,
                                 chassis_move_t *chassis_move_rc_to_vector);
 ...
 */
static void
chassis_zero_force_control(fp32 *vx_can_set, fp32 *vy_can_set, fp32 *wz_can_set,
                           chassis_move_t *chassis_move_rc_to_vector);
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode) {
  ...
  if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL])) {
    chassis_behaviour_mode = CHASSIS_AUTO;
                 ...
                 chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL])) {
  ...
  if (chassis_behaviour_mode == CHASSIS_ZERO_FORCE) {
    ...
    chassis_move_mode->chassis_mode = CHASSIS_VECTOR_RAW; //当行为是底盘无力，则设置底盘状态机为
                                                          //raw，原生状态机。
  } else if (chassis_behaviour_mode == CHASSIS_NO_MOVE) {

    chassis_move_mode->chassis_mode =
        CHASSIS_VECTOR_SPEED; //当行为是底盘不移动，则设置底盘状态机为
                              //底盘不跟随角度 状态机。
  } else if (chassis_behaviour_mode == CHASSIS_AUTO) {

    chassis_move_mode->chassis_mode =
        CHASSIS_VECTOR_SPEED; //当行为是底盘不跟随角度，则设置底盘状态机为
                              //底盘不跟随角度 状态机。
  } else if (chassis_behaviour_mode == CHASSIS_NORMAL_MODE) {
    chassis_move_mode->chassis_mode =
        CHASSIS_VECTOR_SPEED; //当行为是底盘开环，则设置底盘状态机为 底盘原生raw
                              //状态机。
  }
}
