...
static const uint8_t cali_name[CALI_LIST_LENGTH][3] = {"HD", "GM", "GYR", "ACC",
                                                       "MAG"}; //校准设备的名字

//校准设备对应放入结构体变量地址
static uint32_t *cali_sensor_buf[CALI_LIST_LENGTH] = {
    (uint32_t *)&head_cali, (uint32_t *)&gimbal_cali, (uint32_t *)&gyro_cali,
    (uint32_t *)&accel_cali, (uint32_t *)&mag_cali};

//校准设备对应放入数据大小
static uint8_t cali_sensor_size[CALI_LIST_LENGTH] = {
    sizeof(head_cali_t) / 4, sizeof(gimbal_cali_t) / 4, sizeof(imu_cali_t) / 4,
    sizeof(imu_cali_t) / 4, sizeof(imu_cali_t) / 4};

//校准设备对应的校准函数
void *cali_hook_fun[CALI_LIST_LENGTH] = {cali_head_hook, NULL, cali_gyro_hook,
                                         NULL, NULL};
...
//返回mpu6500需要控制到的温度
int8_t get_control_temperate(void) { return head_cali.temperate; }
...
static void RC_cmd_to_calibrate(void) {
  ...
  if (calibrate_RC->rc.ch[0] < -RC_CALI_VALUE_HOLE &&
      ...
      switch_is_down(calibrate_RC->rc.s[1]) && rc_action_falg == 0) {
  ...
  if (calibrate_systemTick - rc_cmd_systemTick > CALIBRATE_END_TIME) {
    ...
    rc_action_falg = 0;
    return;
  } else if (calibrate_systemTick - rc_cmd_systemTick >
                 RC_CALI_BUZZER_MIDDLE_TIME &&
             rc_cmd_systemTick != 0 && rc_action_falg != 0) {

    //判断遥控器10s后校准选择时间切换蜂鸣器高频声音
    rc_cali_buzzer_middle_on();
  } else if (calibrate_systemTick - rc_cmd_systemTick > 0 &&
             rc_cmd_systemTick != 0 && rc_action_falg != 0) {
...
void cali_data_read(void) {
  ...
  for (i = 0; i < CALI_LIST_LENGTH; i++) {
    ...
    cali_flash_read(FLASH_USER_ADDR + offset, cali_sensor[i].flash_buf,
                    cali_sensor[i].flash_len);
    //偏移校准数据字节大小
    offset += cali_sensor[i].flash_len * 4;

    //读取校准设备的前部数据
    cali_flash_read(FLASH_USER_ADDR + offset, (uint32_t *)flash_read_buf,
                    CALI_SENSOR_HEAD_LEGHT);
    //将名字，校准标识符进行赋值
...
static void cali_data_write(void) {
  ...
  for (i = 0; i < CALI_LIST_LENGTH; i++) {
    //复制设备校准数据
    memcpy((void *)(flash_write_buf + offset), (void *)cali_sensor[i].flash_buf,
           cali_sensor[i].flash_len * 4);
    offset += cali_sensor[i].flash_len * 4;

    //复制设备前部参数，例如名字， 数据大小
    memcpy((void *)(flash_write_buf + offset), (void *)cali_sensor[i].name,
  ...
  //写入flash
  cali_flash_write(FLASH_USER_ADDR, (uint32_t *)flash_write_buf,
                   (FLASH_WRITE_BUF_LENGTH + 3) / 4);
}
