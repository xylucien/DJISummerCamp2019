...
static const uint8_t write_mpu6500_reg_data_error[MPU6500_Write_Reg_Num][3] = {
    {MPU_PWR_MGMT_1,
     (((~MPU_DEVICE_RESET) & (~MPU_SLEEP) & (~MPU_CYCLE) & (~MPU_GYRO_STANDBY) &
       (~MPU_TEMP_DISABLE)) &
      (MPU_CLKSEL_INTERNAL)),
     PWR_MGMT_1_ERROR},

    {MPU_PWR_MGMT_2,
     (((~MPU_DISABLE_XA) & (~MPU_DISABLE_YA) & (~MPU_DISABLE_ZA) &
       (~MPU_DISABLE_XG) & (~MPU_DISABLE_YG) & (~MPU_DISABLE_ZG)) &
      (MPU_LP_WAKE_1_25_HZ)),
     PWR_MGMT_2_ERROR},
    ...
    {MPU_CONFIG,
     ((~MPU_FIFO_MODE_OFF_REPLACE_OLD_DATA) &
      (MPU_EXT_SYNC_DISABLE | MPU_DLPF_CFG_2_SET)),
     CONFIG_ERROR},

    {MPU_GYRO_CONFIG,
     (((~MPU_XG_SELF_TEST_SET) & (~MPU_YG_SELF_TEST_SET) &
       (~MPU_ZG_SELF_TEST_SET)) &
      (MPU_GYRO_RANGLE)),
     GYRO_CONFIG_ERROR},

    {MPU_ACCEL_CONFIG,
     (((~MPU_XA_SELF_TEST_SET) & (~MPU_YA_SELF_TEST_SET) &
       (~MPU_ZA_SELF_TEST_SET)) &
      (MPU_ACCEL_RANGLE)),
     ACCEL_CONFIG_ERROR},

    {MPU_ACCEL_CONFIG_2, (MPU_ACCEL_FCHOICE_B_0_SET | MPU_A_DLPL_CFG_0_SET),
     ACCEL_CONFIG_2_ERROR},

    {MPU_WOM_THR, WOM_THR_Set, MOT_DETECT_CTRL_ERROR},

    {MPU_I2C_MST_CTRL,
     (((~MPU_MULT_MST_EN) & (~MPU_SLV_3_FIFO_EN) & (~MPU_I2C_MST_P_NSR)) &
      (MPU_WAIT_FOR_ES_EN | MPU_I2C_MST_CLK_400_KHZ)),
     I2C_MST_CTRL_ERROR},

    {MPU_INTBP_CFG,
     ((~MPU_INTBP_OPEN) & (~MPU_LATCH_INT_EN) & (~MPU_INT_ANYRD_2CLEAR) &
      (~MPU_ACTL_FSYNC) & (~MPU_FSYNC_INT_MODE_EN)) &
         (MPU_INTBP_ACTL | MPU_BYPASS_EN),
     INTBP_CFG_ERROR},
    ...
    {MPU_USER_CTRL,
     (((~MPU_DMP_EN) & (~MPU_FIFO_MODE_EN) & (~MPU_DMP_RST) & (~MPU_FIFO_RST) &
       (~MPU_I2C_MST_RST) & (~MPU_SIG_COND_RST)) &
...
uint8_t mpu6500_init(void) {
  ...
  for (writeNum = 0; writeNum < MPU6500_Write_Reg_Num; writeNum++) {
    mpu6500_write_single_reg(write_mpu6500_reg_data_error[writeNum][0],
                             write_mpu6500_reg_data_error[writeNum][1]);
    mpu6500_delay_us(wait_time);
...
void mpu6500_read_over(uint8_t *status_buf,
  ...
  if ((*status_buf) & MPU_RAW_RDY_INT) {
    ...
    mpu6500_real_data->temp =
        temp_imu_data * MPU6500_TEMPERATURE_FACTOR + MPU6500_TEMPERATURE_OFFSET;
...
void gyro_offset_calc(fp32 gyro_offset[3], fp32 gyro[3], uint8_t imu_status,
